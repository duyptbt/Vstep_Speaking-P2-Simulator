    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-2xl p-5 sm:p-6 shadow-md space-y-5`}>
            <div className={`flex flex-wrap items-center justify-between gap-2 border-b ${theme.cardBorder} pb-3`}>
              <span className="text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400 flex items-center gap-1.5">
                <FileText className="w-4 h-4" />
                VSTEP Part 2 Situation
              </span>
              <div className="flex items-center gap-2">
                <TtsSpeedControl isLight={theme.isLight} />
                <button
                  onClick={() => speakText(`Situation: ${question.situation}`)}
                  className={`px-3 py-1.5 ${theme.btnSecondary} rounded-xl text-xs font-semibold transition flex items-center gap-1.5 cursor-pointer`}
                >
            ...
            <div className={`${theme.subCardBg} border ${theme.subCardBorder} rounded-xl p-4`}>
              <h3 className={`text-base sm:text-lg font-bold ${theme.textPrimary} mb-2`}>{question.situationTitle}</h3>
              <p className={`text-xs sm:text-sm ${theme.textSecondary} leading-relaxed font-medium`}>
                {question.situation}
              </p>
            </div>
            ...
            <div className="space-y-3">
              ...
              {question.options.map((opt) => (
                <div
                  key={opt.id}
                  className={`${theme.isLight ? "bg-slate-50 border-slate-200" : "bg-slate-950/80 border-slate-800"} border rounded-xl p-4 space-y-2 hover:border-slate-400 dark:hover:border-slate-700 transition`}
                >
                  <div className="flex items-center justify-between">
                    <span className="px-2.5 py-0.5 rounded bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30 font-bold text-xs">
                      {opt.label}
                    </span>
                    <span className={`text-xs font-bold ${theme.textPrimary}`}>{opt.title}</span>
                  </div>
                  <p className={`text-xs ${theme.textSecondary} leading-normal`}>{opt.description}</p>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 text-[11px]">
                    <div className={`${theme.isLight ? "bg-emerald-50 border-emerald-200 text-emerald-900" : "bg-emerald-950/30 border-emerald-800/30 text-emerald-300"} border rounded p-2`}>
                      <strong className="block text-emerald-700 dark:text-emerald-400 font-semibold mb-0.5">Pros:</strong>
                    ...
                    <div className={`${theme.isLight ? "bg-rose-50 border-rose-200 text-rose-900" : "bg-rose-950/30 border-rose-800/30 text-rose-300"} border rounded p-2`}>
                      <strong className="block text-rose-700 dark:text-rose-400 font-semibold mb-0.5">Cons:</strong>
                      <ul className="list-disc list-inside space-y-0.5">
            ...
            <div className={`${theme.isLight ? "bg-indigo-50/80 border-indigo-200" : "bg-indigo-500/10 border-indigo-500/30"} border rounded-xl p-4`}>
              <span className="text-xs font-bold text-indigo-700 dark:text-indigo-300 uppercase tracking-wider block mb-1">
                Prompt:
              </span>
              <p className="text-xs sm:text-sm font-bold text-indigo-950 dark:text-indigo-100">
                "{question.prompt}"
              </p>
            </div>
          </div>
          ...
          <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-2xl p-5 sm:p-6 shadow-md space-y-3`}>
            <div className={`flex items-center justify-between border-b ${theme.cardBorder} pb-3`}>
              <div className="flex items-center space-x-2">
                <Edit3 className="w-4 h-4 text-amber-500" />
                <h3 className={`text-sm font-bold ${theme.textPrimary} uppercase tracking-wider`}>
              ...
              {practiceNotes && (
                <button
                  ...
                  className={`p-1 ${theme.textMuted} hover:text-rose-500 transition cursor-pointer`}
                  title="Clear Notes"
                >
            ...
            <textarea
              ...
              } border rounded-xl p-4 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono leading-relaxed resize-none shadow-inner`}
            />
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-2xl p-5 shadow-md space-y-4`}>
            <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b ${theme.cardBorder} pb-3`}>
              <div className="flex items-center space-x-2">
                <Sparkles className="w-4 h-4 text-amber-500" />
                <span className={`text-xs font-bold ${theme.textPrimary} uppercase tracking-wider`}>
                  Target Proficiency Language Toolkit
                </span>
              </div>

              {/* Band B1 vs B2 Switcher */}
              <div className={`flex items-center ${theme.isLight ? "bg-slate-100 border-slate-300" : "bg-slate-950 border-slate-800"} p-1 rounded-xl border`}>
                <button
            ...
            <div className={`p-3.5 rounded-xl border text-xs leading-relaxed ${
              selectedBand === "B1"
                ? theme.isLight ? "bg-blue-50 border-blue-200 text-blue-950" : "bg-blue-950/40 border-blue-800/40 text-blue-200"
                : theme.isLight ? "bg-emerald-50 border-emerald-200 text-emerald-950" : "bg-emerald-950/40 border-emerald-800/40 text-emerald-200"
            }`}>
              <div className="font-bold flex items-center gap-1.5 mb-1">
                <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold uppercase ${
                  selectedBand === "B1" ? "bg-blue-600 text-white" : "bg-emerald-600 text-white"
                }`}>
          ...
          <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-2xl p-5 sm:p-6 shadow-md space-y-3`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <Sparkles className={`w-4 h-4 ${selectedBand === "B1" ? "text-blue-500" : "text-emerald-500"}`} />
                <span className={`text-xs font-bold ${theme.textPrimary} uppercase tracking-wider`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
              {(selectedBand === "B1"
                ...
                <div
                  key={i}
                  className={`${theme.isLight ? "bg-slate-50 border-slate-200" : "bg-slate-950/80 border-slate-800/80"} border rounded-xl p-2.5 flex flex-col justify-between space-y-1 hover:border-slate-400 dark:hover:border-slate-700 transition`}
                >
                  <div className="flex items-start justify-between gap-1.5">
                    ...
                    {item.type && (
                      <span className={`text-[9px] px-1.5 py-0.5 rounded ${theme.badgeBg} ${theme.textMuted} flex-shrink-0 font-medium`}>
                        {item.type}
                      </span>
                    )}
                  </div>
          ...
          <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-2xl p-5 sm:p-6 shadow-md space-y-3`}>
                ...
                {(selectedBand === "B1"
                  ...
                  <span
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-medium border ${
                      selectedBand === "B1"
                        ? theme.isLight ? "bg-blue-50 border-blue-200 text-blue-900" : "bg-blue-950/40 border-blue-800/40 text-blue-200"
                        : theme.isLight ? "bg-emerald-50 border-emerald-200 text-emerald-900" : "bg-emerald-950/40 border-emerald-800/40 text-emerald-200"
                    }`}
                  >
                    {phrase}
                  </span>
            ...
            <div className={`space-y-2.5 pt-2 border-t ${theme.cardBorder}`}>
              <span className={`text-[11px] font-bold ${theme.textMuted} uppercase tracking-wider block`}>
              ...
              {(selectedBand === "B1"
                ...
                <div key={i} className={`${theme.isLight ? "bg-slate-50 border-slate-200" : "bg-slate-950/60 border-slate-800/80"} border rounded-xl p-3 space-y-1.5`}>
                  <span className={`text-xs font-bold block ${
                    selectedBand === "B1" ? "text-blue-600 dark:text-blue-300" : "text-emerald-600 dark:text-emerald-300"
                  }`}>
                    {stageItem.stage}
                  </span>
                  <ul className={`space-y-1 text-xs ${theme.textSecondary}`}>
                    {stageItem.templates.map((tpl, j) => (
                      <li key={j} className="flex items-start gap-1.5">
                        <span className={`${theme.textMuted} font-mono text-[11px] mt-0.5`}>•</span>
          ...
          <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-2xl p-5 sm:p-6 shadow-md space-y-3`}>
            ...
            <ul className={`space-y-2 text-xs sm:text-sm ${theme.textSecondary}`}>
              {(selectedBand === "B1"
                ? question.languageInputB1?.responseFormula || question.tips
                : question.languageInputB2?.responseFormula || question.tips
              ).map((tip, i) => (
                <li key={i} className={`flex items-start gap-2.5 ${theme.isLight ? "bg-slate-50 border-slate-200" : "bg-slate-800/50 border-slate-700/50"} p-2.5 rounded-lg border`}>
          ...
          <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-2xl overflow-hidden shadow-md`}>
            <button
              onClick={() => setIsPronunciationOpen(!isPronunciationOpen)}
              className={`w-full px-5 py-3.5 ${
                theme.isLight ? "bg-indigo-50 hover:bg-indigo-100/80 text-indigo-900 border-indigo-200" : "bg-indigo-950/40 hover:bg-indigo-950/60 text-indigo-300 border-indigo-500/20"
              } font-bold text-xs sm:text-sm flex items-center justify-between border-b transition cursor-pointer`}
            >
              <div className="flex items-center space-x-2">
            ...
            {isPronunciationOpen && (
              <div className={`p-5 space-y-4 text-xs sm:text-sm ${theme.textSecondary}`}>
                <div className={`space-y-2 border-b ${theme.cardBorder} pb-3`}>
                  ...
                  <p className={`font-mono ${theme.isLight ? "text-emerald-700 bg-slate-50 border-slate-200" : "text-emerald-400 bg-slate-950 border-slate-800"} px-3 py-2 rounded-lg border text-xs`}>
                    {(selectedBand === "B1"
                  ...
                  <p>
                    <strong className={theme.textPrimary}>Intonation Contour:</strong>{" "}
                    {(selectedBand === "B1"
                  ...
                  <p>
                    <strong className={theme.textPrimary}>Stress & Linking:</strong>{" "}
                    {(selectedBand === "B1"
                ...
                <div className={`space-y-1.5 ${theme.textSecondary}`}>
          ...
          <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-2xl p-5 sm:p-6 shadow-md space-y-4`}>
            <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b ${theme.cardBorder} pb-3`}>
              <div className="flex items-center space-x-2">
                <Award className={`w-4 h-4 ${selectedBand === "B1" ? "text-blue-500" : "text-emerald-500"}`} />
                <span className={`text-xs font-bold ${theme.textPrimary} uppercase tracking-wider`}>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold transition flex items-center gap-1.5 shadow-md flex-shrink-0 text-white cursor-pointer ${
                    selectedBand === "B1"
            ...
            <div
              className={`${
                theme.isLight ? "bg-slate-50 border-slate-200 text-slate-800" : "bg-slate-950 border-slate-800 text-slate-200"
              } border rounded-xl p-4 text-xs sm:text-sm leading-relaxed font-sans whitespace-pre-line border-l-4 ${
          ...
          <div className={`${theme.cardBg} border ${theme.cardBorder} rounded-2xl p-5 sm:p-6 shadow-md space-y-4`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center space-x-2">
                ...
                <span className={`text-xs font-bold ${theme.textPrimary} uppercase tracking-wider`}>Record Practice Response</span>
              </div>
              ...
              <div className="flex items-center space-x-2">
                {recordingState === "idle" && (
                  <button
                    ...
                    className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold rounded-xl transition flex items-center gap-1.5 shadow-md cursor-pointer"
                  >
                ...
                {recordingState === "recording" && (
                  <button
                    ...
                    className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-rose-400 text-xs font-bold rounded-xl transition flex items-center gap-1.5 border border-rose-500/40 animate-pulse cursor-pointer"
                  >
                ...
                {recordingState === "recorded" && (
                  <>
                    <button
                      ...
                      className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition flex items-center gap-1.5 cursor-pointer"
                    >
                    ...
                    <button
                      onClick={startPracticeRecording}
                      className={`px-3.5 py-2 ${theme.btnSecondary} text-xs font-semibold rounded-xl transition flex items-center gap-1 cursor-pointer`}
                    >
            ...
            <div className={`text-xs ${
              theme.isLight ? "bg-slate-50 border-slate-200 text-slate-800" : "bg-slate-950 border-slate-800 text-slate-300"
            } p-3.5 rounded-xl border font-mono min-h-[60px]`}>
              {practiceTranscript ? (
                <span>{practiceTranscript}</span>
              ) : (
                <span className={`${theme.textMuted} italic`}>Click "Record Answer" above to practice speaking into your microphone.</span>
              )}
            </div>
          </div>
